...
function Type(i) {
  ...
  if (pokemons[i].type[n] == "grass") {
    tipos = tipos + `<p class="planta">Planta</p> `
  }
  ...
  if (pokemons[i].type[n] == "steel") {
    tipos = tipos + `<p class="acero">Acero</p> `
  }
...
for (let i = 0; i < pokemons.length; i++) {
  if (i == 0) {
    poke = poke + `<div type="button" class="btn btn-lg btn-gradient btn-danger imagenes col" data-bs-toggle="popover" data-bs-title="${pokemons[i].name} - ${pokemons[i].number}" data-bs-content="Peso : ${pokemons[i].weight} Kg - Altura : ${pokemons[i].height} cm - Debilidades : ${Debilidad(i)} - Habilidades : [${pokemons[i].abilities}] " data-bs-custom-class="custom-popover"> 
    ...
    if (pokemons[i].ThumbnailImage == pokemons[i-1].ThumbnailImage){
      ...
      poke = poke + `<div type="button" class="btn btn-lg btn-gradient btn-danger imagenes col" data-bs-toggle="popover" data-bs-title="${pokemons[i].name} - ${pokemons[i].number}" data-bs-content="Peso : ${pokemons[i].weight} Kg - Altura : ${pokemons[i].height} cm - Debilidades : ${Debilidad(i)} - Habilidades : [${pokemons[i].abilities}] "> 
      <img src="${pokemons[i].ThumbnailImage}"> 
      <p class="numero"> N°.${pokemons[i].number}</p>
      <h3 class="nombre"> ${pokemons[i].name}</h3>
      ${Type(i)}    
...
document.getElementById("Cards").innerHTML = poke
...
buscar.addEventListener("click", (e) => {
    ...
    alert("Introduzca un nombre o numero de un pokemon valido")
    }
    else if ((Number(busqueda.value)) >= 1){
      let i = motorDeBusqueda("number", (Number(busqueda.value)))
      let pokemon = `<div type="button" class="resultado btn btn-lg btn-gradient btn-danger imagenes col"data-bs-toggle="popover">  
      <img src="${pokemons[i].ThumbnailImage}"> 
      <p class="numero"> N°.${pokemons[i].number}</p>
      <h2 class="nombre"> ${pokemons[i].name}</h2>
      <h5>${Type(i)} </h5>
      <p>Peso: ${pokemons[i].weight} Kg - Altura: ${pokemons[i].height} cm - Debilidades: ${Debilidad(i)} - Habilidades: [${pokemons[i].abilities}] </p>
